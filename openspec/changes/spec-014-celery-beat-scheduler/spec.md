...
(Previously: No beat schedule configured)

#### Scenario: Beat schedule interval configured

- GIVEN `app/worker.py` has `beat_schedule` setting
- WHEN Celery Beat initializes
- THEN `check_scheduled_posts` is scheduled with 60-second interval
- AND scheduler persists in-memory (no `--schedule` flag for MVP)

---

### Requirement: Check Scheduled Posts Task

The system MUST provide a `check_scheduled_posts` Celery task that queries due posts and dispatches processing.

The task SHALL query posts with status=PENDING and scheduled_at <= now(), update status to PROCESSING, and dispatch `process_instagram_post`.

#### Scenario: Task queries and dispatches

- GIVEN posts exist in PENDING state with past scheduled_at
- WHEN `check_scheduled_posts.delay()` is called (or triggered by beat)
- THEN task queries database for eligible posts
- AND each post status transitions PENDING → PROCESSING
- AND `process_instagram_post.delay(post_id)` is called for each
...
- AND logs indicate "0 posts found for scheduling"
