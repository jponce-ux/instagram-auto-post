...
- [ ] **T1.1** Create Alembic migration for `email_logs` table
...
- [ ] **T1.2** Create `EmailLog` SQLAlchemy model
...
- [ ] **T1.3** Register model in `app/models/__init__.py`
...
- [ ] **T2.1** Update `EmailService.send_welcome_email()` to create log
...
- [ ] **T2.2** Add idempotency check to EmailService
  - File: `app/services/email.py`
...
- [ ] **T2.3** Pass `log_id` to Celery task
  - File: `app/services/email.py`
